...
- **Use kebab-case for all file names.** Never use camelCase or PascalCase for file names — use kebab-case instead (e.g., `use-novels.ts`, `auth-middleware.ts`, `query-client.tsx`).
- **No inline `useQuery`/`useMutation` in components.** Always extract them into reusable custom hooks (e.g., `use-novels.ts`, `use-favorites.ts`). Components should only call these hooks, never `useQuery`/`useMutation` directly.
